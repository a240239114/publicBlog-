...
//comment集合
var es6List = require('../models/es6List')
//引入mongodb
...
MongoClient.connect(url, {
    ...
    router.get('/', async (req, res) => {
        //数据库中查找所有数据es6List集合查找
        if (err) throw err;
        //获取数据库
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("es6List").find({}).toArray(function (err, data) { // 返回集合中所有数据
            if (err) {
                res.json({
                    status: 301,
                    msg: "不好意思"
    ...
    router.get('/count/count', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("es6List").countDocuments({}, function (err, data) { // 返回集合中所有数据
            if (err) {
                res.json({
                    status: 301,
                    msg: "不好意思"
    ...
    router.get('/index/:index', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("es6List").find().skip(8 * (index - 1)).limit(8).toArray(function (err, data) { // 返回集合中所有数据
            if (err) throw err;
            res.json({
                data
            })
    ...
    router.post('/', async (req, res) => {
        //数据库中查找所有数据"es6List集合查找
        if (err) throw err;
        var dbo = db.db("publicBlog");
        //查询自增前的es6Listid的counters
        var data = await dbo.collection("counters").find({
            _id: "es6Listid"
        }).toArray();
        var sequence_value = data[0].sequence_value;
        //自增函数
        req.body["_id"] = await getNextSequenceValue("es6Listid", db.db("publicBlog"));


        if (req.body["_id"] != sequence_value) {
            //自动添加next信息
            // req.body["nextId"] = parseInt(req.body["_id"] + 1);
            // const data = await dbo.collection("es6List").find({
            //     _id: req.body["nextId"]
            // }).toArray();
            ...
            dbo.collection("es6List").insertOne(req.body, function (err, data) {
                if (err) throw err;
                console.log("文档插入成功");
                // db.close();
            });
    ...
    router.post('/:id', async (req, res) => {
        ...
        if (err) throw err;
        var dbo = db.db("publicBlog");
        dbo.collection("es6List").updateOne({
            _id: id
        }, {
            $set: req.body
        }, function (err, obj) {
