...
MongoClient.connect(url, {
    ...
    router.get('/', async (req, res) => {
        //数据库中查找所有数据jsList集合查找
        if (err) throw err;
        //获取数据库
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("jsList").find({}).toArray(function (err, data) { // 返回集合中所有数据
            if (err) {
                res.json({
                    status: 301,
                    msg: "不好意思"
       ...
       router.get('/count/count', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("jsList").countDocuments({},function (err, data) { // 返回集合中所有数据
            if (err) {
                res.json({
                    status: 301,
                    msg: "不好意思"
    ...
    router.get('/index/:index', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("jsList").find().skip(8 * (index - 1)).limit(8).toArray(function (err, data) { // 返回集合中所有数据
            if (err) throw err;
            res.json({
                data
            })
    ...
    router.post('/', async (req, res) => {
        //数据库中查找所有数据"jsList集合查找
        if (err) throw err;
        var dbo = db.db("publicBlog");
        //查询自增前的jsListid的counters
        var data = await dbo.collection("counters").find({
            _id: "jsListid"
        }).toArray();
        var sequence_value = data[0].sequence_value;
        //自增函数
        req.body["_id"] = await getNextSequenceValue("jsListid", db.db("publicBlog"));


        if (req.body["_id"] != sequence_value) {
            //自动添加next信息
            // req.body["nextId"] = parseInt(req.body["_id"] + 1);
            // const data = await dbo.collection("jsList").find({
            //     _id: req.body["nextId"]
            // }).toArray();
            ...
            dbo.collection("jsList").insertOne(req.body, function (err, data) {
                if (err) throw err;
                console.log("文档插入成功");
                // db.close();
            });
    ...
    router.post('/:id', async (req, res) => {
        ...
        if (err) throw err;
        var dbo = db.db("publicBlog");
        dbo.collection("jsList").updateOne({
            _id: id
        }, {
            $set: req.body
        }, function (err, obj) {
    ...
    router.delete('/id/:id', async (req, res) => {
        ...
        //删除当前的数据 ID
        dbo.collection("jsList").deleteOne({
            _id: id
        }, function (err, obj) {
            if (err) throw err;
            res.json({
    ...
    router.delete('/tittle/:tittle', async (req, res) => {
        ...
        //删除当前的数据 
        dbo.collection("jsList").deleteMany({
            tittle: tittle
        }, async function (err, obj) {
            var count = await dbo.collection("jsList").find().count();
            if (count == 0) {
                getToZeroSequenceValue("jsListid", db.db("publicBlog"))
            }
