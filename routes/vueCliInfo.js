...
MongoClient.connect(url, {
    ...
    router.get('/', async (req, res) => {
        ...
        //操作数据库中的集合
        dbo.collection("vueCliInfo").find({}).toArray(function (err, data) { // 返回集合中所有数据
            if (err) {
                res.json({
                    status: 301,
                    msg: "不好意思"
    ...
    router.get('/count/count', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("vueCliInfo").countDocuments({}, function (err, data) { // 返回集合中所有数据
            console.log(data);
            if (err) {
                res.json({
                    status: 301,
    ...
    router.post('/:id', async (req, res) => {
        ...
        if (err) throw err;
        var dbo = db.db("publicBlog");
        dbo.collection("vueCliInfo").updateOne({
            _id: id
        }, {
            $set: req.body
        }, function (err, obj) {
    ...
    router.delete('/tittle/:tittle', async (req, res) => {
        ...
        dbo.collection("vueCliInfo").deleteMany({
            ...
            var count = await dbo.collection("vueCliInfo").find().count();
            if (count == 0) {
                getToZeroSequenceValue("vueCliInfoid", db.db("publicBlog"))
            }
