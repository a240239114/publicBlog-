...
MongoClient.connect(url, {
    ...
    router.get('/count/count', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("vueList").countDocuments({}, function (err, data) { // 返回集合中所有数据
            if (err) {
                res.json({
                    status: 301,
                    msg: "不好意思"
    ...
    router.post('/', async (req, res) => {
        ...
        //自增函数
        req.body["_id"] = await getNextSequenceValue("vueCliListid", db.db("publicBlog"));
    ...
    router.delete('/tittle/:tittle', async (req, res) => {
        ...
        dbo.collection("vueCliList").deleteMany({
            ...
            var count = await dbo.collection("vueCliList").find().count();
            if (count == 0) {
