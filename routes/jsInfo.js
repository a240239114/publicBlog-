...
//comment集合
var jsInfo = require('../models/jsInfo')
//引入mongodb
...
MongoClient.connect(url, {
    useNewUrlParser: true
      ...
      router.get('/', async (req, res) => {
        ...
        //操作数据库中的集合
        dbo.collection("jsInfo").find({}).toArray(function (err, data) { // 返回集合中所有数据
            if (err) {
                res.json({
                    status: 301,
                    msg: "不好意思"
    ...
    router.get('/count/count', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("jsInfo").countDocuments({}, function (err, data) { // 返回集合中所有数据
            console.log(data);
            if (err) {
                res.json({
                    status: 301,
    ...
    router.post('/', async (req, res) => {
        ...
        //自增函数
        req.body["_id"] = await getNextSequenceValue("jsInfoid", db.db("publicBlog"));
        ...
        if (req.body["_id"] != sequence_value) {
            if (sequence_value != 0) {
                ...
                const data = await dbo.collection("jsInfo").find({
                    _id: req.body["lastId"]
                }).toArray();
            ...
            dbo.collection("jsInfo").insertOne(req.body, function (err, data) {
                if (err) throw err;
                console.log("文档插入成功");
                // db.close();
            });
    ...
    router.post('/:id', async (req, res) => {
        ...
        if (err) throw err;
        var dbo = db.db("publicBlog");
        dbo.collection("jsInfo").updateOne({
            _id: id
        }, {
            $set: req.body
        }, function (err, obj) {
    ...
    router.delete('/id/:id', async (req, res) => {
        ...
        //删除当前的数据 ID
        dbo.collection("jsInfo").deleteOne({
            _id: id
        }, function (err, obj) {
            if (err) throw err;
            res.json({
    ...
    router.delete('/tittle/:tittle', async (req, res) => {
        ...
        //删除当前的数据 
        dbo.collection("jsInfo").deleteMany({
            tittle: tittle
        }, async function (err, obj) {
            var count = await dbo.collection("jsInfo").find().count();
            if (count == 0) {
                getToZeroSequenceValue("jsInfoid", db.db("publicBlog"))
            }
