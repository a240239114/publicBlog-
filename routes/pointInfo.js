...
//comment集合
var pointInfo = require('../models/pointInfo')
//引入mongodb
...
MongoClient.connect(url, {
    ...
    router.get('/', async (req, res) => {
        ...
        //操作数据库中的集合
        dbo.collection("pointInfo").find({}).toArray(function (err, data) { // 返回集合中所有数据
            if (err) {
                res.json({
                    status: 301,
                    msg: "不好意思"
    ...
    router.get('/count/count', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("pointInfo").countDocuments({}, function (err, data) { // 返回集合中所有数据
            console.log(data);
            if (err) {
                res.json({
                    status: 301,
    ...
    router.get('/:id', async (req, res) => {
        //数据库中查找所有数据,pointInfo集合查找
        let id = parseInt(req.params.id);
        if (err) throw err;
        //获取数据库
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("pointInfo").find({
            _id: id
    ...
    router.post('/', async (req, res) => {
        ...
        //自增函数
        req.body["_id"] = await getNextSequenceValue("pointInfoid", db.db("publicBlog"));
        ...
        if (req.body["_id"] != sequence_value) {
            if (sequence_value != 0) {
                ...
                const data = await dbo.collection("pointInfo").find({
                    _id: req.body["lastId"]
                }).toArray();
            ...
            dbo.collection("pointInfo").insertOne(req.body, function (err, data) {
                if (err) throw err;
                console.log("文档插入成功");
                // db.close();
            });
    ...
    router.post('/:id', async (req, res) => {
        ...
        if (err) throw err;
        var dbo = db.db("publicBlog");
        dbo.collection("pointInfo").updateOne({
            _id: id
        }, {
            $set: req.body
        }, function (err, obj) {
    ...
    router.delete('/id/:id', async (req, res) => {
        ...
        //删除当前的数据 ID
        dbo.collection("pointInfo").deleteOne({
            _id: id
        }, function (err, obj) {
            if (err) throw err;
            res.json({
    ...
    router.delete('/tittle/:tittle', async (req, res) => {
        ...
        //删除当前的数据 
        dbo.collection("pointInfo").deleteMany({
            tittle: tittle
        }, async function (err, obj) {
            var count = await dbo.collection("pointInfo").find().count();
            if (count == 0) {
                getToZeroSequenceValue("pointInfoid", db.db("publicBlog"))
            }
