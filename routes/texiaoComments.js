...
router.get('/',async (req,res)=>{
    ...
    const data = await texiaoComments.find()
    res.json({data})
})
...
router.get('/:id',async (req,res)=>{
    ...
    //分页查询
    const data = await texiaoComments.find().limit(5).skip(5*(id-1))
    // const data = await texiaoComments.find()
    res.json({data})
})
...
router.post('/',async (req,res)=>{
    ...
    // await texiaoComments.create(req.body)
    new texiaoComments(req.body).save()
