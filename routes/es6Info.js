...
MongoClient.connect(url, {
    useNewUrlParser: true
      ...
      router.get('/', async (req, res) => {
        ...
        //操作数据库中的集合
        dbo.collection("es6Info").find({}).toArray(function (err, data) { // 返回集合中所有数据
            if (err) {
                res.json({
                    status: 301,
                    msg: "不好意思"
    ...
    router.get('/count/count', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("es6Info").countDocuments({}, function (err, data) { // 返回集合中所有数据
            console.log(data);
            if (err) {
                res.json({
                    status: 301,
    ...
    router.delete('/tittle/:tittle', async (req, res) => {
        ...
        dbo.collection("es6Info").deleteMany({
            ...
            var count = await dbo.collection("es6Info").find().count();
            if (count == 0) {
