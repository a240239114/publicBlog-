...
//comment集合
var bugInfo = require('../models/bugInfo')
//引入mongodb
...
MongoClient.connect(url, {
    useNewUrlParser: true
        ...
        router.get('/', async (req, res) => {
            ...
            //操作数据库中的集合
            dbo.collection("bugInfo").find({}).toArray(function (err, data) { // 返回集合中所有数据
                if (err) {
                    res.json({
                        status: 301,
                        msg: "不好意思"
        ...
        router.get('/count/count', async (req, res) => {
            ...
            var dbo = db.db("publicBlog");
            //操作数据库中的集合
            dbo.collection("bugInfo").countDocuments({}, function (err, data) { // 返回集合中所有数据
                console.log(data);
                if (err) {
                    res.json({
                        status: 301,
    ...
    router.post('/', async (req, res) => {
        ...
        //自增函数
        req.body["_id"] = await getNextSequenceValue("bugInfoid", db.db("publicBlog"));
        ...
        if (req.body["_id"] != sequence_value) {
            if (sequence_value != 0) {
                ...
                const data = await dbo.collection("bugInfo").find({
                    _id: req.body["lastId"]
                }).toArray();
            ...
            dbo.collection("bugInfo").insertOne(req.body, function (err, data) {
                if (err) throw err;
                console.log("文档插入成功");
                // db.close();
            });
    ...
    router.post('/:id', async (req, res) => {
        ...
        if (err) throw err;
        var dbo = db.db("publicBlog");
        dbo.collection("bugInfo").updateOne({
            _id: id
        }, {
            $set: req.body
        }, function (err, obj) {
    ...
    router.delete('/id/:id', async (req, res) => {
        ...
        //删除当前的数据 ID
        dbo.collection("bugInfo").deleteOne({
            _id: id
        }, function (err, obj) {
            if (err) throw err;
            res.json({
    ...
    router.delete('/tittle/:tittle', async (req, res) => {
        ...
        //删除当前的数据 
        dbo.collection("bugInfo").deleteMany({
            tittle: tittle
        }, async function (err, obj) {
            var count = await dbo.collection("bugInfo").find().count();
            if (count == 0) {
                getToZeroSequenceValue("bugInfoid", db.db("publicBlog"))
            }
