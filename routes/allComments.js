...
MongoClient.connect(url, {
    ...
    router.get('/', async (req, res) => {
        ...
        //操作数据库中的集合
        dbo.collection("allComments").find({}).toArray(function (err, data) { // 返回集合中所有数据
            if (err) throw err;
            res.json({
                data
            })
    ...
    router.get('/:id', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("allComments").find().skip(8 * (id - 1)).limit(8).toArray(function (err, data) { // 返回集合中所有数据
            if (err) throw err;
            res.json({
                data
            })
    ...
    router.post('/', async (req, res) => {
        ...
        //自增函数
        req.body["_id"] = await getNextSequenceValue("allCommentsid", db.db("publicBlog"));

        console.log(req.body);


        if (req.body["_id"] != sequence_value) {
            dbo.collection("allComments").insertOne(req.body, function (err, data) {
                if (err) throw err;
                console.log("文档插入成功");
                // db.close();
            });
    ...
    router.delete('/:id', async (req, res) => {
        ...
        //删除当前的数据 ID
        dbo.collection("allComments").deleteOne({
            _id: id
        }, function (err, obj) {
            if (err) throw err;
            res.json({
    ...
    router.delete('/', async (req, res) => {
        ...
        //删除所有数据
        dbo.collection("allComments").deleteMany({
            _id: {
                $gte: 0
            }
        }, function (err, obj) {
            ...
            //SequenceValue归零
            getToZeroSequenceValue("allCommentsid", db.db("publicBlog"))
            // db.close();
            res.json({
                status: 202,
                msg: '全部删除成功'
