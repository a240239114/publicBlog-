...
MongoClient.connect(url, {
    ...
    router.get('/', async (req, res) => {
       ...
       router.get('/count/count', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("h5c3List").countDocuments({},function (err, data) { // 返回集合中所有数据
            if (err) {
                res.json({
                    status: 301,
                    msg: "不好意思"
    ...
    router.get('/index/:index', async (req, res) => {
        ...
        var dbo = db.db("publicBlog");
        //操作数据库中的集合
        dbo.collection("h5c3List").find().skip(8 * (index - 1)).limit(8).toArray(function (err, data) { // 返回集合中所有数据
            if (err) throw err;
            res.json({
                data
            })
    ...
    router.post('/:id', async (req, res) => {
        ...
        if (err) throw err;
        var dbo = db.db("publicBlog");
        dbo.collection("h5c3List").updateOne({
            _id: id
        }, {
            $set: req.body
        }, function (err, obj) {
    ...
    router.delete('/id/:id', async (req, res) => {
        ...
        //删除当前的数据 ID
        dbo.collection("h5c3List").deleteOne({
            _id: id
        }, function (err, obj) {
            if (err) throw err;
            res.json({
    ...
    router.delete('/tittle/:tittle', async (req, res) => {
        ...
        //删除当前的数据 
        dbo.collection("h5c3List").deleteMany({
            tittle: tittle
        }, async function (err, obj) {
            var count = await dbo.collection("h5c3List").find().count();
            if (count == 0) {
                getToZeroSequenceValue("h5c3Listid", db.db("publicBlog"))
            }
