...
MongoClient.connect(url, {
    ...
    router.post('/', async (req, res) => {
        ...
        //自增函数
        req.body["_id"] = await getNextSequenceValue("bugCommentsid",db.db("local"));

        console.log(req.body);


        if (req.body["_id"] != sequence_value) {
            dbo.collection("bugComments").insertOne(req.body, function (err, data) {
                if (err) throw err;
                console.log("文档插入成功");
                // db.close();
            });
